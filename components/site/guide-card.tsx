  ...
  return (
    <Card className="overflow-hidden">
      ...
      <div className="space-y-4 p-6">
        <div>
          <h3 className="font-serif text-2xl font-semibold"><Link href={`/guides/${guide.slug}`}>{guide.name}</Link></h3>
        ...
        <Button asChild variant="outline">
          <Link href={`/guides/${guide.slug}`}>Meet the guide <ArrowRight className="ml-2 h-4 w-4" /></Link>
        </Button>
      </div>
    </Card>
  );
