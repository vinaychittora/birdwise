...
export function ReportCard({ report }: { report: FieldReport }) {
  const href = `/field-reports/${report.slug}` as Route;
  ...
  return (
    <Card className="overflow-hidden">
      ...
      <div className="space-y-4 p-6">
        ...
        <div>
          <h3 className="font-serif text-2xl font-semibold"><Link href={href}>{report.title}</Link></h3>
          <p className="mt-2 text-sm leading-7 text-foreground/75">{report.summary}</p>
        </div>
        <Button asChild variant="outline">
          <Link href={href}>Read report <ArrowRight className="ml-2 h-4 w-4" /></Link>
        </Button>
      </div>
    </Card>
  );
